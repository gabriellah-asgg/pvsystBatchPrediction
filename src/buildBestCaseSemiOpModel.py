...
for model in model_params.keys():
    ...
    if not skip_model:
        if 'tuned' in model:
            train_model, rmse_score, si_score, model_names, rmse_list, si_list, best_params = tune_models(curr_model, curr_param, X_train,y_train, X_test, y_test, model_names, rmse_list, si_list,verbose=5)
            ...
            train_model, rmse_score, si_score, model_names, rmse_list, si_list = build_models(curr_model, X_train,y_train, X_test, y_test, model_names, rmse_list, si_list)
            score = "_" + str(round(rmse_score, 2))
