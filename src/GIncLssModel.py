...
filepath = r'Q:\Projects\224008\DESIGN\ANALYSIS\00_PV\PVsyst Batch Simulation\ginclss_data.xlsx'
model_df = construct_df(filepath, columns=["Sheds Tilt", "Sheds Azim", "GIncLss (KWh)"])

# standardize data
scaler = StandardScaler()
y = model_df['GIncLss (KWh)']
X = model_df.drop(['GIncLss (KWh)'], axis=1)
X_scaled = scaler.fit_transform(X)
pv_type = filepath.split('\\')[-1].replace(".xlsx", "")
export_model(scaler, 'ginclss_data', False)
...
for model in model_params.keys():
    ...
    skip_model = check_models_to_run(curr_model, curr_param, pv_type, filepath=r"../res/ginclss_data/ginclss_cache.json")
    if not skip_model:
        if 'tuned' in model:
            ...
            export = add_model_params(pv_type, curr_param, model, rmse_score, si_score, best_params, filepath=r"../res"
                                                                                                              r"/ginclss_data/ginclss_cache.json")
            ...
            if model_params.get(model).get("fit_params"):
                ...
                add_model_params(pv_type, all_params, model, rmse_score, si_score, filepath=r"../res/ginclss_data"
                                                                                            r"/ginclss_cache.json")
                ...
                train_model, rmse_score, si_score, model_names, rmse_list, si_list = build_models(curr_model, X_train,
                                                                                                  ...
                                                                                                  rmse_list, si_list)
                score = "_" + str(round(rmse_score, 2))
                add_model_params(pv_type, curr_param, model, rmse_score, si_score, filepath=r"../res/ginclss_data"
                                                                                            r"/ginclss_cache.json")

            export_model(train_model, pv_type, False, score)
export_to_csv(pv_type, filepath=r"../res/ginclss_data/ginclss_cache.json")
