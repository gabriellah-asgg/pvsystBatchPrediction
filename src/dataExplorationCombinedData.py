...
df = preprocessor.read_worksheet(skip_rows=11, columns=["Indent", "Sheds Tilt", "Sheds Azim","Comment","EArray","Syst_ON","EArrNom","GIncLss","TempLss","ModQual","OhmLoss","MisLoss","EArrMPP","EffArrR","EffSysR","EffSysC"])
...
model_df = numeric_data.drop(['Syst_ON', 'MisLoss', 'EffSysC'], axis=1)

# scatterplot
for column in model_df.columns:
    plt.figure()
    plt.scatter(model_df[column], model_df['EArray'])
    plt.title(column + " vs EArray (KWh)")
    plt.xlabel(column)
    plt.ylabel('EArray (KWh)')
    plt.savefig(r'C:\Users\gabriellahoover\Pictures\pvsyst_charts\combined_data\earray_vs_' + column)
    ...
    plt.ylabel(column)
    plt.savefig(r'C:\Users\gabriellahoover\Pictures\pvsyst_charts\combined_data\tilt_vs_' + column)
    ...
    plt.ylabel(column)
    plt.savefig(r'C:\Users\gabriellahoover\Pictures\pvsyst_charts\combined_data\azim_vs_' + column)
plt.show()
